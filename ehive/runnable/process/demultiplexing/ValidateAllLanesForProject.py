...
class ValidateAllLanesForProject(IGFBaseProcess):
  ...
  def _check_project_table(self,project_status,barcode_check_key='barcode_check',\
    ...
    :param project_status: Project status info, PASS or FAIL
    :param barcode_check_key: The attribute key name for barcode check, default barcode_check
    :param barcode_off_value: A keyword for barcode check setting, default OFF
    '''
    try:
      db_connected = 0
      ...
      if project_status=='FAIL':
        ...
        result = \
          base.fetch_records(\
            query,
            output_mode='one_or_none')                                          # fetch data from db
          ...
          for flowcell in os.listdir(report_dir):
            ...
            if os.path.isdir(flowcell_dir):
              ...
              if os.path.exists(all_barcodes_html):
                ...
                message = \
                  'Failed barcode stats for seqrun: {0}, project: {1}'.\
