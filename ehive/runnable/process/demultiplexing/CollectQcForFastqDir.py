...
class CollectQcForFastqDir(IGFBaseProcess):
  def param_defaults(self):
    params_dict=super(CollectQcForFastqDir,self).param_defaults()
    params_dict.update({
  ...
  def run(self):
    try:
      ...
      fastqc_files = \
        [fqc_file
          for fqc_file in fastqc_info.values() \
      ...
      if remote_fastqs_info is not None:
        for fq_file, fqs_file in fastqscreen_info.items():
          if fq_file in remote_fastqs_info.keys():
            ...
            fqs_file.\
              update({'remote_fastqscreen_path':remote_fqs_file,
                      'fastq_file':fq_file})                                    # add remote fastqscreen path

      fastqscreen_files = \
        [fsr_file
          for fsr_file in fastqscreen_info.values() \
