...
class UploadAnalysisResultsToIrods(IGFBaseProcess):
  ...
  def run(self):
    ...
    try:
      ...
      pa.start_session()
      user=pa.fetch_data_authority_for_project(project_igf_id=project_igf_id)   # fetch user info from db
      pa.close_session()
      ...
      irods_upload.upload_analysis_results_and_create_collection(\
        file_list=file_list,
        irods_user=username,
      ...
      message='project: {2}, Error in {0}: {1}'.format(self.__class__.__name__, \
                                                       e, \
                                                       project_igf_id)
