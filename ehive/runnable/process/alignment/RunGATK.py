...
class RunGATK(IGFBaseProcess):
  ...
  def run(self):
    ...
    try:
      ...
      if gatk_cmd not in gatk_allowed_commands:
      ...
      gatk_obj=GATK_tools(gatk_exe=gatk_exe,
      ...
      if gatk_command == 'BaseRecalibrator':
        ...
        output_table=os.path.join(work_dir,
                                  '{0}_{1}.table'.\
                                  format(os.path.basename(input_bam),
                                         gatk_command))
        gatk_cmdline=gatk_obj.\
        ...
        self.param('dataflow_params',{'BaseRecalibrator_table':output_table})   # pass on bqsr output list

      elif gatk_command == 'ApplyBQSR':
        bqsr_recal_file=self.param_required('bqsr_recal_file')
        input_bam=self.param_required('input_bam')
        output_bam_path=os.path.join(work_dir,
                                  '{0}_{1}.bam'.\
                                  format(os.path.basename(input_bam).\
                                           replace('.bam',''),
                                         gatk_command))
        gatk_cmdline=gatk_obj.\
            run_ApplyBQSR(\
              ...
              force=force_overwrite)
        self.param('dataflow_params',{'ApplyBQSR_bam':output_bam_path})         # pass on apply bqsr bam
        ...
        output_pdf_path=os.path.join(work_dir,
                                  '{0}_{1}.pdf'.\
                                  format(os.path.basename(before_report_file).\
                                           replace('.table',''),
                                         gatk_command))
        gatk_cmdline=gatk_obj.\
            run_AnalyzeCovariates(\
               ...
               force=force_overwrite)
        self.param('dataflow_params',{'AnalyzeCovariates_pdf':output_pdf_path}) # pass on apply bqsr bam

      elif gatk_command == 'HaplotypeCaller':
        input_bam=self.param_required('input_bam')
        emit_gvcf=self.param('hc_gvcf')
        output_vcf_path=os.path.join(work_dir,
                                  '{0}_{1}.g.vcf'.\
                                  format(os.path.basename(before_report_file).\
                                           replace('.bam',''),
                                         gatk_command))
        ...
        if emit_gvcf:
          self.param('dataflow_params',{'HaplotypeCaller_gvcf':output_vcf_path})# pass on hc gvcf
        else:
          self.param('dataflow_params',{'HaplotypeCaller_vcf':output_vcf_path}) # pass on hc vcf

      else:
        raise ValueError('Gatk command {0} not supported'.format(gatk_command))

      message='Finished GATK {0} for {1}: {2}'.\
