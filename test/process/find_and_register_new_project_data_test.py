...
class Find_and_register_project_data1(unittest.TestCase):
  def setUp(self):
    ...
    new_project_data = [
      ...
      {'project_igf_id':'IGFP0003_test_24-8-2017_rna',
       ...
       'barcode_check':'OFF'}]
    pd.DataFrame(new_project_data).\
      to_csv(os.path.join('.',self.new_project_data))
  ...
  def test_read_project_info_and_get_new_entries(self):
    ...
    all_data = \
      fa._read_project_info_and_get_new_entries(
        project_info_file=self.new_project_data)
    project_data = \
      all_data['project_data'].to_dict(orient='records')
    self.assertEqual(
      project_data[0]['project_igf_id'],
      'IGFP0002_test_23-5-2017_rna')
    self.assertEqual(
      project_data[0]['barcode_check'],
      'ON')
    self.assertEqual(
      project_data[1]['project_igf_id'],
      'IGFP0003_test_24-8-2017_rna')
    self.assertEqual(
      project_data[1]['barcode_check'],
      'OFF')
    sample_data = \
      all_data['sample_data'].to_dict(orient='records')
    self.assertEqual(
      sample_data[0]['sample_igf_id'],
      'IGF00006')
    user_data = \
      all_data['user_data'].to_dict(orient='records')
    self.assertEqual(
      user_data[0]['email_id'],
      'user2@ic.ac.uk')
    project_user_data = \
      all_data['project_user_data'].to_dict(orient='records')
    self.assertEqual(
      project_user_data[0]['email_id'],
      'user2@ic.ac.uk')
    self.assertEqual(
      project_user_data[0]['project_igf_id'],
      'IGFP0002_test_23-5-2017_rna')
  ...
  def test_process_project_data_and_account(self):
      ...
      ua = UserAdaptor(**{'session':base.session})
      user_exists = \
        ua.check_user_records_email_id(
          email_id='user2@ic.ac.uk')
      self.assertTrue(user_exists)
      user1 = \
        ua.fetch_user_records_email_id(
          user_email_id='user2@ic.ac.uk')
