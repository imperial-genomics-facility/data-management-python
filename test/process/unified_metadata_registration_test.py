...
class TestUnifiedMetadataRegistrationA(unittest.TestCase):
  ...
  @patch(
    "igf_data.process.seqrun_processing.unified_metadata_registration.get_data_from_portal",
    return_value={"A": '[{"project_igf_id": "A"}]'}
  )
  ...
  def test_CheckRawMetadataColumnsCommand_check_columns(self):
    ...
    self.assertEqual(
      ...
      ",".join(table_columns_dict.get('user'))
    )
    metadata_columns = [
      "project_igf_id",
      "name",
    ...
    self.assertEqual(
      str(errors[0]),
      "Missing required columns in sample table: " +
      ",".join(table_columns_dict.get('sample'))
    )
  ...
  def test_CheckRawMetadataColumnsCommand_execute(self):
    ...
    # # self.assertEqual(
    # #   len(metadata_context.error_list), 0
    # # )
    # # metadata_context.raw_metadata_dict = {
    ...
    self.assertEqual(
      len(metadata_context.checked_required_column_dict), 1) # 2)
    ...
    self.assertTrue(
      "A" in metadata_context.checked_required_column_dict
    )
    ...
    # self.assertEqual(
    #   metadata_context.checked_required_column_dict.get("A"), True
    # )
    ...
    self.assertEqual(
      len(metadata_context.checked_required_column_dict), 1
    )
    # # self.assertTrue(1 in metadata_context.checked_required_column_dict)
    # # self.assertEqual(metadata_context.checked_required_column_dict.get(1), True)
    self.assertTrue(
  ...
  def test_CheckAndRegisterMetadataCommand_split_metadata(self):
    metadata_context = MetadataContext(
      ...
      checked_required_column_dict={"A": True}, #, 2: False},
      validated_metadata_dict={"A": True}
    )
    # 2: pd.DataFrame([{
    ...
    entries = check_and_register._split_metadata(
      metadata_entry=metadata_context.raw_metadata_dict.get("A"),
    ...
    (_, _, _, sample_metadata_list) = check_and_register._split_metadata(
      metadata_entry=metadata_context.raw_metadata_dict.get("A"),
      table_columns=self.table_columns,
      samples_required=False
    )
  ...
  def test_CheckAndRegisterMetadataCommand_check_and_filter_existing_metadata(self):
    session_class = _get_db_session_class(
      db_config_file=self.db_config_file
    )
    check_and_register = CheckAndRegisterMetadataCommand()
    (
      ...
      _
    ) = check_and_register._check_and_filter_existing_metadata(
      project_metadata_list=[
        {"project_igf_id": "IGFP0001_test_22-8-2017_rna"},
  ...
  def test_CheckAndRegisterMetadataCommand_register_update_projectuser_metadata(self):
    check_and_register = CheckAndRegisterMetadataCommand()
    updated_project_user_dataset = check_and_register._update_projectuser_metadata(
      project_user_metadata_list=[
        {"project_igf_id": "IGFP0002_test_22-8-2017_rna", "email_id": "d@b.com"},
    ...
    self.assertTrue([
      e['data_authority'] for e in updated_project_user_dataset
    ...
    self.assertTrue(
      "c@c.com" in [e["email_id"] for e in updated_project_user_dataset
  ...
  def test_CheckAndRegisterMetadataCommand_register_new_metadata(self):
    ...
    check_and_register = CheckAndRegisterMetadataCommand()
    updated_project_user_metadata = check_and_register._update_projectuser_metadata(
      project_user_metadata_list=[
        {"project_igf_id": "IGFP0002_test_22-8-2017_rna", "email_id": "d@b.com"}],
      secondary_user_email='c@c.com'
    )
    status, _ = check_and_register._register_new_metadata(
      project_metadata_list=[
        {"project_igf_id": "IGFP0002_test_22-8-2017_rna"}],
      user_metadata_list=[
        {"name": "DB DB", "email_id": "d@b.com", "username": "db"}],
...
class TestUnifiedMetadataRegistrationB(unittest.TestCase):
  ...
  def test_UnifiedMetadataRegistration_execute(self, *args):
    ...
    self.assertEqual(len(metadata_context.synced_metadata_dict), 1)
    self.assertTrue("A" in metadata_context.synced_metadata_dict)
    # self.assertTrue(2 in metadata_context.synced_metadata_dict)
    self.assertTrue(metadata_context.synced_metadata_dict.get("A"))
    # self.assertFalse(metadata_context.synced_metadata_dict.get(2))
...
if __name__ == '__main__':
  unittest.main()
  # test = TestUnifiedMetadataRegistrationA()
  # test.setUp()
  # test.test_CheckRawMetadataColumnsCommand_execute()