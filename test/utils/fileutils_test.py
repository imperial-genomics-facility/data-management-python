...
class Fileutils_test1(unittest.TestCase):
  def setUp(self):
    ...
    self.results_dir = \
      get_temp_dir()
    self.output_tar_file = \
      os.path.join(
        get_temp_dir(),
        'test.tar')
    self.output_targz_file = \
      os.path.join(
        get_temp_dir(),
        'test.tar.gz')
    self.manifest_file = \
    ...
    for f in self.test_file_list:
      with open(os.path.join(self.work_dir, f), 'w') as fp:
        fp.write('A')
  ...
  def test_tar_output(self):
    prepare_file_archive(
      results_dirpath=self.results_dir,
  ...
  def test_create_file_manifest_for_dir(self):
    ...
    html_data = \
      manifest_data[
        manifest_data['file_path']=='web_summary.html']
  ...
  def test_change_file_and_dir_permission(self):
    ...
    try:
      ...
      self.fail("Failed to unlock file")
    change_file_and_dir_permission(
    ...
    try:
      ...
      self.fail("Failed to unlock dir")
