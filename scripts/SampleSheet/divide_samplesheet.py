...
if len(sample_lane) > 1:
  ...
  for lane_id in sample_lane:
    ...
    data_group=samplesheet_data_tmp.group_data_by_index_length()
...
for index_length in data_group.keys():
  output_file=os.path.join(output_dir, '{}.csv'.format(index_length))
  data_group[index_length].print_sampleSheet(outfile=output_file)
