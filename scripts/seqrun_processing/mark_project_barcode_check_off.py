...
if __name__=='__main__':
  try:
    ...
    if log_slack:
      message = \
        'Found {0} new projet ids to switch off barcode checking'.\
    ...
    for project_id in project_id_list:
      try:
        ...
        message = \
          'Switching off barcode checking for project {0}'.\
            format(project_id)
        if log_slack:
          slack_obj.post_message_to_channel(message, reaction='pass')
        ...
        message = \
          'Failed to reset barcode checking as OFF for project {0}'.\
            format(project_id)
        if log_slack:
