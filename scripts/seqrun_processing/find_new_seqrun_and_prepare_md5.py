...
if __name__=='__main__':
  try:
    ...
    if len(new_seqruns.keys()) > 0:
      message = \
        'found {0} new sequence runs, calculating md5'.\
          format(len(new_seqruns.keys()))
      slack_obj.post_message_to_channel(message,reaction='pass')

      new_seqrun_files_and_md5 = \
        calculate_file_md5(
          seqrun_info=new_seqruns,
          md5_out=md5_path,
          seqrun_path=seqrun_path,
          exclude_dir=exclude_path)
      slack_obj.post_message_to_channel(
        message='finished md5 calculation, loading seqrun to db',
        reaction='pass')

      load_seqrun_files_to_db(
        seqrun_info=new_seqruns,
        seqrun_md5_info=new_seqrun_files_and_md5,
        dbconfig=dbconfig_path)
      ...
      for seqrun_name in new_seqruns.keys():
        ...
        slack_obj.post_message_to_channel(message,reaction='pass')
        message = \
          'New asana task created for seqrun {0}, url: https://app.asana.com/0/{1}/{2}'.\
            format(
              seqrun_name,
              asana_project_id,
              res['target']['gid'])
      ...
      slack_obj.post_message_to_channel(
        message='No new sequencing run found',reaction='sleep')
