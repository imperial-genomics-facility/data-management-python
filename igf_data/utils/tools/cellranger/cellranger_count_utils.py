...
def run_cellranger_multi(
  ...
  """
  A function for running Cellranger multi tool

  :param cellranger_exe: Path to Cellranger exe
  :param library_csv: Path to library.csv file
  :param sample_id: Sample id
  :param output_dir: Output path
  :param use_ephemeral_space: A toggle for using EPHEMERAL temp space, default False
  :param job_timeout: Job timeout, default 43200
  :param cellranger_options: Cellranger multi options, default: ('--localcores 1','--localmem 8')
  :returns: two strings, cmd and output_dir
  """
  try:
    tmp_dir = \
      get_temp_dir(
...
def _check_cellranger_multi_output(cellranger_output, library_csv, sample_id):
  """
  Internal function for checking cellranger v6 multi output

  :param cellranger_output: Cellranger output path
  :param library_csv: path to cellranger multi csv
  :param sample_id: Sample id, for non cell-multiplexing samples
  :returns: None
  """
  try:
    #file_check_dict = {
...
def get_cellranger_count_input_list(
      db_session_class, experiment_igf_id, fastq_collection_type='demultiplexed_fastq',
  ...
  '''
  try:
    fastq_dir_list=get_fastq_input_list(
    ...
    raise ValueError(
            'Failed to get cellranger count output list, error: {0}'.format(e))


def check_cellranger_count_output(
      output_path,
      file_list=(
        'web_summary.html',
        'metrics_summary.csv',
  ...
  try:
    ...
    file_dict_value = [
      i for i in range(len(file_list))]                                         # get file index values
    ...
    if len(list(file_dict.keys())) > 0:
      raise IOError(
              'Partial Cellranger count output found, missing following files: {0}'.\
                format(list(file_dict.keys())))
  except Exception as e:
    raise ValueError('Failed to check cellranger output, error: {0}'.format(e))


def extract_cellranger_count_metrics_summary(
      cellranger_tar, collection_name=None, collection_type=None,
      attribute_name='attribute_name', attribute_value='attribute_value',
      attribute_prefix='None', target_filename='metrics_summary.csv'):
  '''
  ...
  try:
    ...
    if metrics_file is None:
      raise IOError(
              'Required file {0} not found in tar {1}'.\
    ...
    raise ValueError(
            'Failed to extract cellranger tar, error: {0}'.format(e))