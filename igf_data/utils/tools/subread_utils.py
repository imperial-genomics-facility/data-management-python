...
def run_featureCounts(featurecounts_exe,input_gtf,input_bams,output_file,thread=1,options=None):
  '''
  ...
  try:
    ...
    featureCount_cmd=[quote(featurecounts_exe),
                      '-a',quote(input_gtf),
                      '-o',quote(temp_output),
                      '-T',quote(str(thread))
                      ]
    ...
    copy_local_file(source_path=temp_output,
                    ...
                    force=True)
    copy_local_file(source_path=temp_summary,
