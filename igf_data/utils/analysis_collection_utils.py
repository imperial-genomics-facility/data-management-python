...
class Analysis_collection_utils:
  ...
  '''
  def __init__(
        self,dbsession_class,base_path=None,collection_name=None,
  ...
  def create_or_update_analysis_collection(
    ...
    try:
      ...
      raise ValueError(
              'Failed to create or update analysis collection, error: {0}'.\
              format(e))
  ...
  def _calculate_file_size_md5(
    ...
    try:
      ...
      raise ValueError(
              'Failed to calculate file size and md5, error: {0}'.\
              format(e))
    ...
    try:
      ...
      if self.base_path is not None:
        if self.collection_table == 'sample':
          ...
          sample_exists = \
            sa.check_sample_records_igf_id(sample_igf_id=sample_igf_id)
          if not sample_exists:
            raise ValueError(
                    'Sample {0} not found in db'.format(sample_igf_id))
          project_igf_id = \
          ...
          if not experiment_exists:
            raise ValueError(\
                    'Experiment {0} not present in database'.\
          ...
          run_exists = \
            ra.check_run_records_igf_id(run_igf_id=run_igf_id)
          if not run_exists:
            raise ValueError(
                    'Run {0} not found in database'.format(run_igf_id))
          ...
          if not project_exists:
            raise ValueError(
                    'Project {0} not found in database'.\
        ...
        if self.base_path is None:                                              # do not move file if base_path is absent
          ...
          if self.collection_table == 'project':
            if project_igf_id is None:
              raise ValueError(
                      'Missing project id for collection {0}'.\
            ...
            if project_igf_id is None or \
              ...
              raise ValueError(
                      'Missing project and sample id for collection {0}'.\
                      format(self.collection_name))
            final_path = \
              os.path.join(
                self.base_path,
            ...
            if project_igf_id is None or \
              ...
              raise ValueError(
                      'Missing project,sample and experiment id for collection {0}'.\
                      format(self.collection_name))
            final_path = \
              os.path.join(
                self.base_path,
            ...
            if project_igf_id is None or \
               ...
               run_igf_id is None:
              raise ValueError(
                      'Missing project,sample,experiment and run id for collection {0}'.\
                      format(self.collection_name))
            final_path = \
              os.path.join(\
                self.base_path,
  ...
  def get_new_file_name(self,input_file,file_suffix=None):
    ...
    try:
      ...
      if new_filename =='':
        raise ValueError(
                'New filename not found for input file {0}'.\
                format(input_file))
      new_filename = \
        '{0}_{1}'.format(
          new_filename,
      ...
      if file_suffix is None:
        file_suffix = \
          get_file_extension(input_file=input_file)                             # collect file suffix
      if file_suffix =='':
        raise ValueError(
                'Missing file extension for new file name of {0}'.\
      ...
      raise ValueError(
              'Failed to get new filename, error: {0}'.format(e))
