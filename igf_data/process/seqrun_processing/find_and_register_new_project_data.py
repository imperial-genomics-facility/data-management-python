...
class Find_and_register_new_project_data:
  ...
  def process_project_data_and_account(self):
    ...
    '''
    try:
      new_project_info_list = \
        self._find_new_project_info()
      if len(new_project_info_list) == 0:
        if self.log_slack:
          self.igf_slack.post_message_to_channel(
            message='No project info found',
            reaction='sleep')
        ...
        try:
          new_data = \
            self._read_project_info_and_get_new_entries(
              project_info_file)                                                # get new project, user and samples information
  ...
  def _notify_about_new_user_account(
        self,data,user_col='username',password_col='password',hpc_user_col='hpc_username',
  ...
  def _read_project_info_and_get_new_entries(self,project_info_file):
    ...
    try:
      if fnmatch.fnmatch(project_info_file, '*.csv'):
        project_info_data = \
          pd.read_csv(project_info_file)                                        # read project info data from csv file
        ...
        if self.metadata_sheet_name not in xl.sheet_names:                      # check for required metadata sheet name
          raise ValueError(
                  'Excel file does not have the sheet {0}'.\
                  format(self.metadata_sheet_name))
        project_info_data = \
          xl.parse(self.metadata_sheet_name)                                    # read xls file from the metadata sheet
      else:
          ...
          if fnmatch.fnmatch(file_path, '*.csv') or \
            ...
            file_check = \
              fa.check_file_records_file_path(
                file_path=os.path.join(root_path,file_path))                    # check for filepath in db
            if not file_check:
              new_project_info_list.\
                append(
                  os.path.join(root_path,file_path))                            # collect new project info files
      fa.close_session()                                                        # disconnect db
      return new_project_info_list
    except Exception as e:
      raise ValueError(
              'Failed to find new project info, error: {0}'.format(e))
