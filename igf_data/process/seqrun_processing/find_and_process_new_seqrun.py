...
def find_new_seqrun_dir(path, dbconfig):
  ...
  try:
    ...
    new_seqrun_dir = \
      check_seqrun_dir_in_db(all_seqrun_dir, dbconfig)
    valid_seqrun_dir = \
...
def validate_samplesheet_for_seqrun(seqrun_info,schema_json,output_dir,samplesheet_file='SampleSheet.csv'):
  ...
  try:
    ...
    for seqrun_name, seqrun_path in seqrun_info.items():
      ...
      errors = \
        samplesheet_data.\
          validate_samplesheet_data(schema_json)                                  # validate samplesheet data
      if len(errors)>0:
        error_file = \
          os.path.join(
            output_dir,
        ...
        errors = [
          e if isinstance(e,str) else '{0} :{1}'.format(e.message,e.path)
            for e in errors]
        errors_str = '\n'.join(errors)
        with open(error_file, 'w') as fh:
          fh.write(errors_str)                                                  # write error file
        error_file_list.\
          update({seqrun_name: error_file})
      else:
    ...
    for seqrun_name, seqrun_path in seqrun_info.items():
      ...
      for row in samplesheet_data._data:
        ...
        if not record_exists:
          ...
          if msg == '':
            msg = \
              'missing sample {0} and project {1} for run {2}'.\
      ...
      if not reject_seqrun:
        new_seqrun_info.\
          update({seqrun_name: seqrun_path})                                   # filter db checking failed runs
    base.close_session()
...
def check_finished_seqrun_dir(
      seqrun_dir, seqrun_path,
      required_files=(
        'RTAComplete.txt',
        'SampleSheet.csv',
        'RunInfo.xml')):
    ...
    for seqrun in seqrun_dir:
      ...
      for serun_file in required_files:
        ...
        if not os.path.exists(required_path):
          ...
          skip = 1
      if skip == 0:
        valid_seqruns[seqrun] = \
          os.path.join(seqrun_path,seqrun)
...
def check_seqrun_dir_in_db(all_seqrun_dir, dbconfig):
  ...
  try:
    ...
    new_runs = \
      list(all_runs.difference(existing_runs))
    return new_runs
  except:
    raise

def calculate_file_md5(
      seqrun_info, md5_out, seqrun_path,
      file_suffix='md5.json', exclude_dir=()):
  '''
  ...
  try:
    ...
    for seqrun_name, seqrun_path in seqrun_info.items():
      ...
      for root_path, dirs, files in os.walk(seqrun_path,topdown=True):
        dirs[:] = [
          d for d in dirs
            if d not in exclude_dir]                                    # exclude listed dires from search
        if len(files)>0:
...
def prepare_seqrun_for_db(seqrun_name, seqrun_path, session_class):
  ...
  try:
    ...
    elif (pl_data.model_name == 'NOVASEQ6000'):
      runparameters_file = \
        os.path.join(seqrun_path,'RunParameters.xml')
      runparameters_data = \
        RunParameter_xml(xml_file=runparameters_file)
...
def load_seqrun_files_to_db(
      seqrun_info, seqrun_md5_info, dbconfig, file_type='ILLUMINA_BCL_MD5'):
  '''
  ...
  for seqrun_name, seqrun_path in seqrun_info.items():
    ...
    seqrun_md5_file_data.\
      append({
        'file_path': seqrun_md5_file,
        'location': 'ORWELL',
        'md5': file_md5,
        'size': file_size})
    seqrun_file_collection.\
      append({
        'name': seqrun_name,
        'type': file_type,
        'file_path': seqrun_md5_file})
