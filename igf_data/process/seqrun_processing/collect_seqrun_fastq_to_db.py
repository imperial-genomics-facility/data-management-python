...
class Collect_seqrun_fastq_to_db():
  def __init__(self,fastq_dir,samplesheet_filename,seqrun_igf_id,model_name,session_class):
  ...
  def find_fastq_and_build_db_collection(self):
    try:
      fastq_files_list=_collect_fastq_and_sample_info(self.fastq_dir,\
                                                     self.samplesheet_filename,\
                                                     self.seqrun_igf_id,\
                                                     self.model_name)
      _build_and_store_exp_run_and_collection_in_db(self.session_class,\
                                                   fastq_files_list)
    except:
      raise
    
    
def _get_fastq_and_samplesheet(fastq_dir,samplesheet_filename):
...
def _collect_fastq_and_sample_info(fastq_dir,samplesheet_filename,seqrun_igf_id,\
                                  model_name):
  (samplesheet_file, r1_fastq_list, r2_fastq_list)=\
    get_fastq_and_samplesheet(fastq_dir, samplesheet_filename)
  ...
  for row in samplesheet_data._data:
    ...
    sample_files=link_fastq_file_to_sample(sample_name,r1_fastq_list, \
                                           r2_fastq_list)
...
def _calculate_experiment_run_and_file_info(data,restricted_list):
...
def _build_and_store_exp_run_and_collection_in_db(session_class,fastq_files_list, restricted_list=['10X']):
