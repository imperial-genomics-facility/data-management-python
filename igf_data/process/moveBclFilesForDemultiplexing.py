...
class moveBclFilesForDemultiplexing:
  def __init__(self,input_dir,output_dir,sample_sheet,run_info_xml):
  ...
  def copy_bcl_files(self):
    ...
    for bcl_entity in bcl_files_list:
      ...
      if os.path.isdir(bcl_entity):          
        # copy dir
        shutil.copytree(input_target, output_target)   
      else:
        # copy file
        shutil.copy(input_target, output_target)
  ...
  def _generate_platform_specific_list(self, lane_list=[]):
    ...
    if (re.search(hiseq_pattern, platform_name)):
      ...
      if platform_series.startswith('K'):
        # hiseq4000
        bcl_files_list.extend(['Data/Intensities/s.locs', 'RunInfo.xml','runParameters.xml'])
 
        if len(lane_list):
          for lane in lane_list:
            bcl_files_list.append('Data/Intensities/BaseCalls/{0}'.format(lane))
        else:
      ...
      # NextSeq
      bcl_files_list.extend(['Data','InterOp','RunInfo.xml','RunParameters.xml']) 
    elif(re.search(miseq_pattern, platform_name)):
      # MiSeq
      bcl_files_list.extend(['Data','RunInfo.xml','runParameters.xml'])
    else:
