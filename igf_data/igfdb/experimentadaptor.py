...
class ExperimentAdaptor(BaseAdaptor):
      ...
      (experiment_df, experiment_attr_df)=\
        BaseAdaptor.\
          divide_data_to_table_and_attribute(\
            ...
            attribute_value_column=attribute_value_column)                      # divide data to experiment and adatpor
      ...
      if 'sample_igf_id' in data.columns:
        ...
        data = \
          data.apply(
            sample_map_function,
            axis=1,
            result_type=None)                                                   # map sample id foreign key id
        data.drop(
          'sample_igf_id',
          axis=1,
          inplace=True)
  ...
  def store_experiment_attributes(self,data,experiment_id='',autosave=False):
    ...
    try:
      ...
      self.store_attributes(
        attribute_table=Experiment_attribute,
  ...
  def check_experiment_records_id(
    ...
    try:
      ...
      raise ValueError(
              'Failed to check exp id, error: {0}'.format(e))
  ...
  def fetch_sample_attribute_records_for_experiment_igf_id(
    ...
    try:
      ...
      if attribute_list is not None and \
        ...
        query = \
          query.\
            filter(Sample_attribute.attribute_name.in_(attribute_list))         # look for only specific attributes, if list provided
      results = \
        self.fetch_records(
          query=query,
      ...
      raise ValueError(
              'Failed to sample attributes, error: {0}'.format(e))

  def update_experiment_records_by_igf_id(
        self,experiment_igf_id,update_data,autosave=True):
    '''
    ...
    try:
      if not isinstance(update_data,dict):
        raise AttributeError(
                'Expecting a dictionary with new data for experiment record update and got {0}'.\
      ...
      for update_key in update_data.keys():
        if update_key not in allowed_experiment_columns:
          raise ValueError(
                  'Check your data, column {0} is not part of Experiment table'.\
      ...
      raise ValueError(
              'Failed to update exp record, error: {0}'.format(e))
  ...
  def fetch_project_and_sample_for_experiment(self,experiment_igf_id):
    ...
    try:
      ...
      raise ValueError(
              'Failed to fetch project and sample, error: {0}'.format(e))
  ...
  def fetch_runs_for_igf_id(self,experiment_igf_id,include_active_runs=True,
    ...
    try:
      ...
      raise ValueError(
              'Failed to fetch runs, error: {0}'.format(e))
